...
<?php
	...
	/* Set up CAPTCHA code for the VOIGT-KAMPFF TEST */
	$captcha = False;
	...
	if(isset($_POST['share'])){
		...
		//DEBUG: echo "SHARE ID: ".$_POST['share'];
	...
	if(isset($_POST['status'])){
		...
		if(strlen($status)<=9000){//character limit says it can't go over 9000!!!1
			...
			//$finalstatus = preg_replace("/\{{(.*?)\}}/","<img style='max-width:100%' src=\"$1\" />", $status);
			$finalstatus = " <br/> ".$status;
			...
			if($lastpost!=$finalstatus && trim($finalstatus)!="<br/>  <br/><br/>"){

				/*PHP SecurImage CAPTCHA Processing*/
				...
				if($securimage->check($_POST['captcha_code'])){
					...
					/** CAPTCHA VOIGT-KAMPFF TEST?*//* --mode down below. Just above textarea in Status Update form.*/
					/*echo "YOU FAILED THE VOIGT-KAMPFF TEST!!! <br/> You are dead. *zap*";*/
				}
		...
		/** TL;DR Alert **/ /* --moved down below. Just above the textarea in the status update form */
				...
				<div class="row-fluid">
					<div class="formbox">
							<?php
								if(strlen($status)>9000){//character limit says it can't go over 9000!!!1
?>
		<div id="alert" style="background-color:red; margin-right: 5px;">
...
<?php
							...
							?>
							<?php
								if(isset($_POST['status']) && $captcha==False){//failed the Voigt-Kamff Test
?>
		<div id="alert" style="background-color:red; margin-right: 5px;">
			You failed the Voigt-Kampff Test! You're a bot!!! <br/> *ZAP* You are DEAD!
		</div>
<?php
								}
							?>
						...
						<form id="statusform" action="index.php" method="post">
							<textarea id="status" name="status" class="formtext" type="text" value=""><?php
								if(strlen($status)>9000 || $captcha==False){//character limit says it can't go over 9000!!!1
									echo $status;//leaves the post in the textbox, even though it's too long.
								}
							?></textarea>
							<br/>
							The Voigt-Kampff Test:
							<img height="50" id="captcha" src="securimage/securimage_show.php" alt="CAPTCHA Image" />
							<br class="rwd-break"/>
							says
							&nbsp;<input type="text" id="captcha_code" name="captcha_code" size="10" maxlength="8" />
							<!--a style="display:none;" id="captcharefresh" href="#" onclick="document.getElementById('captcha').src = 'securimage/securimage_show.php?' + Math.random(); return false"><img src="refresh.png" /></a-->
							<br/>
							<br/>
...
<?php
	...
	if($sortby=="popular"){//aka TRENDING
	...
	}elseif($sortby=="random"){//aka RANDOM
...
?>
							...
							<a href="raw.php?id=<?php echo $id;?>" class="colored">raw</a>
						</div>
					</div>
					<div style="clear: both;"></div>      
				  </div>
